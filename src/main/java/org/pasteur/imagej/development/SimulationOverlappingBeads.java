 ...
 * @author benoit
 */
public class SimulationOverlappingBeads {
    ...
    public SimulationOverlappingBeads(DataPhase_ dp,double axialRange,double photonNumber, double background,int patchSize,double max_radius){
            ...
            //for (double u=minZ,uu=-.0,uuu=-.0;k<nb;u+=stepZ,uu+=stepX,uuu+=stepY,k++){
                ...
                if ((patchSize>=psf.length)&&(patchSize>=psf[0].length)){
                    for (int y=0;y<nbDistance;y++){
                        ...
                        double radius=Math.pow(random.nextDouble(),1./2.)*max_radius;//pow(1/2) used for homogeneity in circle
                        ...
                        double py=yy-ry*this.dp.param.xystep;
                        
                        
                        if (k==9){
                            dp.psf.computePSF(xposit[0]-px, yposit[0]-py, zoilposit[0],position+zum*cameraFlip);
                            psf=dp.psf.getPSF();
                        }
                        else if (k==8){
                            //double randZ=radius*random.nextDouble();
                            dp.psf.computePSF(xposit[0], yposit[0], zoilposit[0],position+beadFocus*cameraFlip);
                            psf=dp.psf.getPSF();
                        }
