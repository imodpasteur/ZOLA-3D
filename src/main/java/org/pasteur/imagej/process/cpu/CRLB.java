        ...
        if (matrixParameter==null){
            ...
            if (nb>0){
                ...
                for (double u=-axialRange/2+position;k<nb;u+=stepZ,k++){
                    ...
                    double [][] ppsf=dp.psf.getPSF();
                    im[k]=new double[ppsf.length][];
                    for (int j=0;j<ppsf.length;j++){
                        im[k][j]=new double[ppsf[j].length];
                        for (int jj=0;jj<ppsf[j].length;jj++){
                            im[k][j][jj]=ppsf[j][jj];
                        }
                    }
