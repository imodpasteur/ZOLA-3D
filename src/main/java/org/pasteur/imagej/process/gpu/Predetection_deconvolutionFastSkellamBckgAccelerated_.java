 ...
 * @author benoit
 */
public class Predetection_deconvolutionFastSkellamBckgAccelerated_ {
    ...
    Pointer device_g;//see S. Remmele, J. Hesser, Vector Extrapolation-Based Acceleration of Regularized Richardson Lucy Image Deblurring, Informatik Actuell, pp. 400-404 (2009).
    ...
    public Predetection_deconvolutionFastSkellamBckgAccelerated_(int nbFrame,int width,int height, DataPhase_ dparam,double mini, double maxi, double step,double photonThreshold){
        ...
        device_g = new Pointer();
        cudaResult =cudaMalloc(device_g, wh*range.length*nbFrame * Sizeof.FLOAT);if (cudaResult != cudaError.cudaSuccess){IJ.log("ERROR cuda 8 "+cudaResult);}
    ...
    public boolean pushImage(double [][] image){
        ...
        if (dataAppended<nbFrame){
            ...
            MyVecDouble.initializeVectorToValue(MyCudaStream.getCUstream(streamId), wh, device_background.withByteOffset(wh*dataAppended*Sizeof.FLOAT), (float)5);IJ.log("ERROR: bckg to 5 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    ...
    public double [][] deconvolution_RL(int nb_iter){
        ...
        toto:for (int t=0;t<nb_iter;t++){
            ...
            //if (t<4) dparam.psf.imshow(wh*nbFrame,height, device_tmpFFT1, "o*psf+b "+nb_iter,"FLOAT");
           ...
           if (t>3){
            ...
            //if (t==100) dparam.psf.imshow(wh*range.length,range.length,height, device_tmpFFT2, "weight","FLOAT");//show first image
            ...
            if (t>iterTh){
                IJ.log("WARNING: remove this line: bckg not fitted after 500 iters");
                for (int i=0;i<this.nbFrame;i++){
                    MyVecDouble.initializeVectorToValue(MyCudaStream.getCUstream(streamId), wh, device_tmpFFT2.withByteOffset((wh*depth*i+wh*(depth-1))*Sizeof.FLOAT), (float)1);
                }
            ...
            //if (t%500==0) dparam.psf.imshow(wh*range.length,range.length,height, device_tmpFFT2, "weight "+t,"FLOAT");//show first image
    ...
    public void crossCorrelInitO(int index){
       ...
       cudaResult=JCuda.cudaMemsetAsync(device_tmpFFT2, 0, (long)this.whFFT*(long)2*(long)(depth)*(long)1 * (long)Sizeof.FLOAT,MyCudaStream.getCudaStream_t(streamId));if (cudaResult != cudaError.cudaSuccess){IJ.log("ERROR memset cuda 0");}
           ...
           //dparam.psf.imshow(totalSize*2,(int)Math.sqrt(totalSize), device_convFFT, "device_convFFT 0 "+y,"FLOAT");
            ...
            //dparam.psf.imshow(whFFT*2*5,heightFFT*2, device_tmpFFT1, "crossp","FLOAT");
        ...
        //dparam.psf.imshow(wh*depth,height, device_tmpFFT1, "cross0","FLOAT");
