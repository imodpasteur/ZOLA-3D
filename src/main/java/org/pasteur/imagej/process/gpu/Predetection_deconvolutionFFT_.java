 ...
 * @author benoit
 */
public class Predetection_deconvolutionFFT_ {
    ...
    Pointer device_M; //o * psf stack
    Pointer device_op; //o * psf stack
    ...
    public Predetection_deconvolutionFFT_(int sizeFullImage, DataPhase_ dparam,double mini, double maxi, double step,double photonThreshold){
        ...
        device_op = new Pointer();
        cudaResult =cudaMalloc(device_op, sizeFullImage*sizeFullImage*range.length * Sizeof.FLOAT);if (cudaResult != cudaError.cudaSuccess){IJ.log("ERROR cuda 8 "+cudaResult);}
        ...
        cudaResult = JCuda.cudaMalloc(device_a, range.length*totalSizeUnpadded * Sizeof.FLOAT);
        if (cudaResult != cudaError.cudaSuccess)
        {
            IJ.log("ERROR malloc cuda build 7");return ;
        }
        float [] a=new float [range.length*totalSizeUnpadded];
        for (int i=0;i<range.length*totalSizeUnpadded;i++){
            a[i]=1;
        }
        Pointer host_a=Pointer.to(a);
        JCuda.cudaMemcpyAsync(device_a, host_a, range.length*totalSizeUnpadded* Sizeof.FLOAT, cudaMemcpyKind.cudaMemcpyHostToDevice, MyCudaStream.getCudaStream_t(streamId));
        ...
        for (int i=0;i<o.length;i++){
            ...
            for (int ii=0;ii<o[i].length;ii++){
                for (int iii=0;iii<o[i][ii].length;iii++){
                    ...
                    fullO[i*o[i].length*o[i][ii].length+ii*o[i][ii].length+iii]=v+(float).0001;
                }
            }
...
//    }
    ...
    public boolean setBackgroundMorphologyCPU(double [][] image){
        ...
        //dparam.psf.imshow(totalSizeUnpadded,sizeFullImage, device_background, "bckg","FLOAT");//show first image
    ...
    public boolean setBackgroundMorphology(){
        
                IJ.log("WARNING: CPU does not know background using this function");
        ...
        MyVecDouble.localMean(MyCudaStream.getCUstream(streamId), this.totalSizeUnpadded, this.sizeFullImage, this.sizeFullImage, smoothsize, this.device_tmpFFT, device_fullImage);//here we multiply with sqrt(total size) to avoid to do it after
        
        MyVecDouble.localMinimum(MyCudaStream.getCUstream(streamId), this.totalSizeUnpadded, this.sizeFullImage, this.sizeFullImage, sizePSF, this.device_convFFT, device_tmpFFT);//here we multiply with sqrt(total size) to avoid to do it after
        
        MyVecDouble.localMean(MyCudaStream.getCUstream(streamId), this.totalSizeUnpadded, this.sizeFullImage, this.sizeFullImage, smoothsize, device_tmpFFT, device_convFFT);//here we multiply with sqrt(total size) to avoid to do it after
        
        MyVecDouble.localMaximum(MyCudaStream.getCUstream(streamId), this.totalSizeUnpadded, this.sizeFullImage, this.sizeFullImage, sizePSF, device_convFFT, device_tmpFFT);//here we multiply with sqrt(total size) to avoid to do it after
        
        MyVecDouble.localMean(MyCudaStream.getCUstream(streamId), this.totalSizeUnpadded, this.sizeFullImage, this.sizeFullImage, smoothsize, device_background, device_convFFT);//here we multiply with sqrt(total size) to avoid to do it after
        
        Pointer host_b=Pointer.to(fullB);
        cudaResult =cudaMemcpyAsync(host_b,device_background, totalSizeUnpadded*Sizeof.FLOAT, cudaMemcpyDeviceToHost,MyCudaStream.getCudaStream_t(streamId));
...
//        meanBckg/=count;
    ...
    public double [][] deconvolution_RL(int iter_max){
        ...
        return deconvolution_RL(iter_max,resultCorrelation,true);
    }
    
    
    public double [][] deconvolution_RL(int iter_max,boolean simpleInitialization){
        if (this.resultCorrelation==null){
            resultCorrelation=new float[range.length][sizeFullImage][sizeFullImage];
        }
        return deconvolution_RL(iter_max,resultCorrelation,simpleInitialization);
    }
    ...
    public double [][] deconvolution_RL(int iter_max,float [][][] resultCorrelation,boolean simpleInitialization){
        ...
        if (simpleInitialization){
            ...
            MyVecDouble.initializeDeconvolution(MyCudaStream.getCUstream(streamId), totalSizeUnpadded*range.length,range.length, device_o, device_op,device_M, (float)0.1);
            
        }
        else{
            this.runCrossCorrelationOnGPU();
            //WEIGHT BCKG:
            ...
            MyVecDouble.divScalarFloat(MyCudaStream.getCUstream(streamId), totalSizeUnpadded*range.length, device_op, device_correlImage, (float)Math.sqrt(totalSize));


            cudaResult=jcuda.jcublas.JCublas2.cublasSgemv(handlecublas,CUBLAS_OP_N,totalSizeUnpadded,range.length,device_alpha,device_op,totalSizeUnpadded,device_a,1,device_beta,device_M,1);
        }
        ...
        toto:for (int t=0;t<iter_max;t++){
            ...
            for (int y=range.length-1;y>=0;y--){//range.length;y++){
                ...
                //divide I by (o*psf+b):
                MyVecDouble.addanddivide(MyCudaStream.getCUstream(streamId), totalSizeUnpadded, device_convFFT, device_fullImage , device_M,device_background);
               ...
               //dparam.psf.imshow(totalSize*2,(int)Math.sqrt(totalSize), device_convFFT, "device_convFFT 0 "+y,"FLOAT");
                ...
                //updata o*psf cor current y:
                {
                    
                    
                    cudaResult=JCuda.cudaMemsetAsync(device_convFFT, 0, this.totalSize*2 * Sizeof.FLOAT,MyCudaStream.getCudaStream_t(streamId));if (cudaResult != cudaError.cudaSuccess){IJ.log("ERROR memset cuda 0");}

                    cudaResult=cusparseSsctr(MyCudaStream.getHandleCuSparse(this.streamId), totalSizeUnpadded, device_o.withByteOffset(y*this.totalSizeUnpadded*Sizeof.FLOAT), this.device_sparseIndexPaddingShift2DEven,device_convFFT, CUSPARSE_INDEX_BASE_ZERO);if (cudaResult != cudaError.cudaSuccess){IJ.log("ERROR cusparseDgthr cuda 3");}

                    

                    cudaResult= JCufft.cufftExecC2C(plan, device_convFFT,device_convFFT,JCufft.CUFFT_FORWARD);if (cudaResult != cudaError.cudaSuccess){IJ.log("ERROR fft cuda 6");}


                    MyVecDouble.complexeMulKernel(MyCudaStream.getCUstream(streamId), totalSize, totalSize, device_tmpFFT, device_convFFT, device_dicFFT.withByteOffset(y*this.totalSize*2*Sizeof.FLOAT));


                    JCufft.cufftExecC2C(plan, device_tmpFFT,device_tmpFFT,JCufft.CUFFT_INVERSE);


                    MyVecDouble.makeResultCorrelation(MyCudaStream.getCUstream(streamId), totalSizeUnpadded, totalSizeUnpadded,totalSize*2, device_correlImage, device_tmpFFT, this.device_sparseIndexPaddingShift2DEven);


                    MyVecDouble.updateMandOP(MyCudaStream.getCUstream(streamId), totalSizeUnpadded, device_M, device_op.withByteOffset(y*this.totalSizeUnpadded*Sizeof.FLOAT), device_correlImage, (float)Math.sqrt(totalSize));
                    
                    
                    
                    //here, device_op contains o*psf
